...
## Setup
Create the project directory and change into it by running
```shell
serverless create --template aws-nodejs --path todomvc-api
cd todomvc-api
```
This should've created two files `handler.js` and `serverless.yml`. There's lots of useful in the default serverless.yaml file but we don't need any of it now so you can go ahead and delete anything that's commented out. 
We want to use the `todomvc-api` profile we created earlier so modifiy your `serverless.yml` to look like this:
```yaml
service: todomvc-api
